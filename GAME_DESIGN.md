...
3. 商人で装備購入 → 銅の剣・木の盾
4. 洞窟攻略（2層）→ 探索
5. 洞窟で宝箱を開ける → Ancient Key入手
6. 街に戻る → 王様と会話
7. 準備を整える → 装備・HP・食料の確認
8. フィールドの遺跡へ → Ancient Keyで扉を開く
9. ゲームクリア
...
東の海岸へ向かうと、確かに大きな岩で塞がれた洞窟の入口（洞窟2）がある。力のハンマーを使うと、岩が砕け散り、洞窟2への道が開ける。洞窟2の内部には、洞窟1よりも強力な魔物が徘徊している。苦戦しながらも最深部（2層）へ到達し、宝箱の中から輝く「聖鉱石」を発見する。
...
聖剣と聖盾を装備し、遺跡へ向かう。遺跡1層には強力な魔物が待ち構えている。遺跡2層の最深部、玉座の間に古代竜が鎮座している。竜は冒険者を見ると、「その光…聖剣と聖盾か。ならば勝負だ、人間よ！」と咆哮し、激しい戦いが始まる。
...
15. 洞窟2攻略（1層→2層）※新モンスター3種（リザードマン、ヴァンパイア、トロル）
...
24. 遺跡1層へ進入 ※新モンスター3種（ゴーレム、ダークナイト、リーパー）
...
【システム要素】
• 死亡時：神父の1マス下の位置で復活、Gold/Food半減
• セーブ：神父に話しかける
• ロード：タイトル画面から
• **戦闘報酬**：モンスター撃破時、EXPとGoldに加えて、1/2の確率でFood+10をドロップ（実装済み）
• **レベルアップ**：累積経験値が一定値に達するとレベルアップ（実装済み）
  - 計算式: レベル n に必要な経験値 = 5 × n × (n - 1)
  - 最大レベル: LV99（必要経験値 48,510）
...
| **TOWN**（街） | TOWN | 24x18 | 安全地帯、商人・宿屋・王様がいる |
...
| アイテム | ATK | 価格 | 入手方法 | 状態 |
|---------|-----|------|---------|-----|
| **Wood Sword**（木の剣） ⭐ | **+2** | **35 Gold** | **商人** | **実装済み** |
| Bronze Sword（銅の剣） | +4 | 60 Gold | 商人 | 実装済み |
| **Iron Sword**（鉄の剣） ⭐ | **+6** | **100 Gold** | **商人** | **実装済み** |
...
| アイテム | DEF | 価格 | 入手方法 | 状態 |
|---------|-----|------|---------|-----|
| Wood Shield（木の盾） | +2 | 30 Gold | 商人 | 実装済み |
| **Bronze Shield**（銅の盾） ⭐ | **+4** | **50 Gold** | **商人** | **実装済み** |
| **Iron Shield**（鉄の盾） ⭐ | **+6** | **90 Gold** | **商人** | **実装済み** |
...
- 目的：戦闘機会を増やし、ゲームテンポを向上させる

**実装ファイル**:
- `game_state.js`（ENEMY_DATA に追加）
- `entities/entity_types.js`（ENEMY_KIND に追加、enemyRules 調整済み）
...
**実装内容**:
- 剣：Wood Sword（木の剣 ATK+2、35 Gold）、Bronze Sword（銅の剣 ATK+4、60 Gold）、Iron Sword（鉄の剣 ATK+6、100 Gold）、Holy Sword（聖剣 ATK+10、イベント入手）
- 盾：Wood Shield（木の盾 DEF+2、30 Gold）、Bronze Shield（銅の盾 DEF+4、50 Gold）、Iron Shield（鉄の盾 DEF+6、90 Gold）、Holy Shield（聖盾 DEF+10、イベント入手）
- 商人の販売リストに木/銅/鉄の剣と盾を追加済み
- 聖剣は鍛冶屋が錬成（イベントアイテム、未実装）
- 聖盾は王様が授与（イベントアイテム、未実装）
- 装備の強さ: 木＜銅＜鉄＜聖

**バランス調整**（実装済み）:
- 木の剣：ATK +1→+2、価格 20→35 Gold（初期装備の有用性向上）
- 銅の剣：ATK +2→+4、価格 40→60 Gold（中盤の戦力強化）
- 鉄の剣：価格 80→100 Gold（高級装備の価値明確化）
- 木の盾：価格 35→30 Gold（初期装備の入手しやすさ向上）
- 鉄の盾：価格 70→90 Gold（高級装備の価値明確化）

**実装ファイル**:
- `constants.js`（ITEM, PRICE, ITEM_META に追加済み）
- `data/items.js`（アイテムデータ追加済み）
- `shop.js`（販売リスト更新済み）
- `state/player_state.js`（装備ボーナス計算の更新済み）
...
- 作業: 街への神父配置と会話追加、save_system.jsとlocalStorage保存/読み込み、スタート画面に「続きから」を追加、敗北時の復活/資源減少ロジックを実装
- 効果: 新マップが未完成でもメタ機能を単体テストでき、以降の作業で安全に状態を記録・再現できる

### Step 2: 装備・経済バランスの先行整備
- 目的: 木/銅/鉄/聖系装備（9.5）とストーリーアイテム（力のハンマー/聖鉱石）を先にデータ化し、販売・装備ロジックを安定させてからイベント導入へ進む
- 作業:
  - `constants.js` に新しい武器/盾/ストーリーアイテムIDと装備ボーナス値を追加
  - `data/items.js` で消費/装備/ストーリー全アイテムのメタデータ・価格を整備
  - `state/player_state.js` の装備処理（`useItemByIndex`, `getEffectiveStats`）をメタデータ駆動にリファクタリングし、聖剣/聖盾の装備条件も組み込む
  - `shop.js` をメタデータ参照型に改修し、木/銅/鉄系のみ販売。テスト時は聖剣/聖盾をコンソール付与で動作確認
  - `save_system.js` が新アイテム構成でも破綻しないか確認し、必要ならバージョン管理を更新
- 効果: 序～中盤の装備バランスを早期に確認でき、以降のイベント実装や敵パラメータ調整がスムーズになる

### Step 3: ストーリー進行フラグとイベント骨格
- 目的: 鍛冶屋救出～聖剣錬成～聖盾授与までの進行フラグ（9.6～9.11, 9.8, 9.9）と会話分岐を構築し、マップ実装前にロジック面を固める
- 作業:
  - `state/player_state.js` / `game_state.js` に `blacksmithRescued`, `hasHammer`, `cave2Unlocked`, `hasOre`, `holySwordCreated`, `hasHolyShield`, `dragonDefeated` などのフラグを追加し、セーブ/ロード対象に含める
  - 鍛冶屋/王様/神父のNPCイベント（力のハンマー授与、鉄の剣+聖鉱石→聖剣錬成、聖盾授与）を `game_state.js` に実装
  - `dialogue.js` / `assets/dialogues.json` にフェーズ別セリフとイベントトリガーを追加し、Tキー会話でフェーズ遷移できるようにする
- 効果: フラグ操作だけでイベント単体テストが可能になり、次工程のマップ導線実装時に不具合を切り分けやすくなる

### Step 4: マップ拡張と探索イベント
- 目的: 新マップ群（CAVE2 系・RUINS 系）と岩破壊イベント（9.3, 9.13）を実装し、鍛冶屋救出～聖鉱石入手～遺跡攻略の導線を完成させる
- 作業:
  - `constants.js` に `CAVE2`, `CAVE2_B2`, `RUINS`, `RUINS_B2` のシーンIDを追加し、`map_data.js` で各ASCIIマップを読み込み・遷移設定を実装
  - `map_data.js` / `Game.EVENTS` に洞窟2入口の岩データ・聖鉱石宝箱・遺跡関連イベントを登録し、`movement_controller.js` / `state/occupancy.js` で岩破壊判定とイベント処理を組み込む
  - `entities/entity_types.js`, `data/enemies.js`, `entities/enemy_director.js` を拡張し、シーンごとの敵プール（洞窟1=ゴースト/ウルフ/スケルトン、洞窟2=リザードマン/ヴァンパイア/トロル、遺跡=ゴーレム/ダークナイト/リーパー、ドラゴン固定配置）に差し替える
  - `assets/enemies.png` に新ドット絵を追加し、`sprite_atlas` のインデックスを更新
- 効果: 実際の探索ルートと戦闘が繋がり、ストーリーイベントを通しで検証できる

### Step 5: 終盤コンテンツと総合テスト
- 目的: 古代竜戦（9.12）とエンディング演出（9.15）を実装し、全フェーズを通したプレイとセーブ互換性を確認する
- 作業:
  - `combat.js` に聖剣/聖盾チェックを追加し、`enemy_director` でドラゴンを遺跡地下2階に固定。撃破時は `game_state` のクリアフラグを立てる
  - `renderer/overlay_layer.js` / `input/overlay_controller.js` でエンディング用オーバーレイ表示と入力処理を実装し、旧 `Game.flags.cleared` 依存を置き換える
  - 全工程の回帰テスト（セーブ/ロード、イベント導線、バトル難易度）と `Progress.md` 更新を実施
- 効果: ストーリー完走テストと最終バランス確認ができ、リリース可能な品質に仕上げられる
