...
| フェーズ | フェーズ名 | 到達条件（フラグ） | 王様のセリフ | 鍛冶屋のセリフ | 神父のセリフ | 会話後イベント | 必要アイテム | 成功時メッセージ | 失敗時メッセージ | 獲得アイテム |
|:-------:|-----------|-------------------|------------|-------------|------------|--------------|------------|----------------|----------------|------------|
| **0** | **START** | ゲーム開始時 | 「よく来た、若き冒険者よ...竜が現れた...鍛冶屋が行方不明」 | 「助けてくれて本当に感謝する！...聖鉱石はここにはなかった」 | 「ようこそ、旅の冒険者よ...ここで祈れば旅路を記録できる」 | **王**: `questTalked=true`, `questGiven=true`<br>**鍛**: `finalizeBlacksmithRescue()` | - | **鍛**: 「鍛冶屋は無事に街へ戻っていった。」 | - | - |
| **1** | **BLACKSMITH_RESCUED** | `blacksmithRescued=true` | 「鍛冶屋を助けてくれたそうだな...聖鉱石が欠かせぬ」 | 「命の恩人だ。礼に力のハンマーを渡そう...東の海岸に岩で塞がれた洞窟...」 | （フェーズ0と同じ） | **鍛**: `tryGivePowerHammer()` | インベントリ空き1枠 | 「Power Hammer を受け取った。」 | 「インベントリの空きが必要だ。」 | **力のハンマー** |
| **2** | **ORE_OBTAINED** | `hasOre=true` | 「聖鉱石を見つけたのだな...早く鍛冶屋に届け」 | 「おお、これこそ聖鉱石だ！...**聖剣を鍛えるには鉄の剣も必要だ**」 | （フェーズ0と同じ） | **鍛**: `tryForgeHolySword()` | **鉄の剣** + 聖鉱石 | 「鍛冶屋が聖剣を鍛え上げた！」 | **「鉄の剣を持っていないため、鍛冶が始められない。」**<br>または<br>「聖鉱石を所持していない。」 | **聖剣**<br>（鉄の剣と聖鉱石を消費） |
| **3** | **HOLY_SWORD_CREATED** | `holySwordCreated=true` | 「おお、見事な聖剣だ！...約束通り、聖盾を授けよう」 | 「聖盾は王様に頼んで受け取ってくれ。」 | （フェーズ0と同じ） | **王**: `tryGrantHolyShield()` | インベントリ空き1枠 | 「聖盾を授かった！」 | 「インベントリの整理が必要だ。」 | **聖盾** |
| **4** | **DRAGON_DEFEATED** | `dragonDefeated=true` | 「竜を倒し、島を救ってくれて感謝するぞ！」 | （セリフ定義なし） | （フェーズ0と同じ） | - | - | - | - | - |
