...
- シーン: 4つの相互接続されたマップ (FIELD, TOWN, CAVE_B1, CAVE_B2)
- 言語: 日本語（すべてのUI/メッセージ）
...
- **FIELD（フィールド）** - 島全体。探索、戦闘、各施設への入口がある。遺跡🏛️がゲームクリアの鍵
- **TOWN（街）** - 安全地帯。商人🧙‍♂️がいて、物資の売買ができる。宿屋で休息も可能
- **CAVE_B1（洞窟B1）** - 洞窟の第1階層。宝箱と強力な敵が出現。階段で下層へ
- **CAVE_B2（洞窟B2）** - 洞窟の第2階層。Ancient Keyを持つ宝箱が存在する最深部
...
| ファイル | 行数 | 責務 |
|---------|------|------|
| `data/enemies.js` | 65 | 敵能力値定義：7種類の敵タイプのHP/ATK/DEF/EXP/Gold範囲 |
| `data/items.js` | 62 | アイテム定義：Food、Potion、装備品、Ancient Key等 |
| `data/maps/*.js` | - | マップデータ：4シーンのタイル配置（ASCII行配列） |
...
2. 街に入り、商人から食料やポーション、装備を購入
3. 宿屋主人に話しかけて10Goldで休息し、HPを回復
4. 食料を管理しながら洞窟（CAVE_B1 → CAVE_B2）を探索
   - CAVE_B1入口: フィールドの洞窟入口に接触
   - CAVE_B1→B2: 階段下を使用
   - CAVE_B2→B1: 階段上を使用
   - CAVE_B1出口: 階段上でフィールドに戻る
5. 洞窟B2（最深部）の宝箱から **Ancient Key** を入手
6. 鍵を持って遺跡に到達するとクリア

**クリア条件:**
1. 洞窟B2（CAVE_B2）の宝箱を開いて **Ancient Key** を入手
2. Ancient Keyを所持した状態でフィールドの遺跡に接触
3. ゲームクリア！
...
- **CAVE_B1/B2**: 各階層で常時2-4体を維持
- **スポーン条件**: プレイヤーから距離4以上離れた場所にのみ出現
- **ドラゴン**: FIELD固定位置に出現し、倒すと再出現しない（`dragonDefeated`フラグ）
...
| FIELD → CAVE_B1 | 洞窟入口触接 | CAVE_B1出口下1タイル |
| CAVE_B1 → FIELD | 洞窟出口触接 | FIELD洞窟入口下1タイル |
| CAVE_B1 → CAVE_B2 | 階段下触接 | CAVE_B2階段上の下1タイル |
| CAVE_B2 → CAVE_B1 | 階段上触接 | CAVE_B1階段下の下1タイル |
...
**開発段階**: MVP完成、コアゲームループ実装済み、リファクタリング完了
