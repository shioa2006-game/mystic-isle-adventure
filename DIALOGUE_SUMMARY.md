...
### フェーズ 1: BLACKSMITH_RESCUED（力のハンマー授与前）
```
鍛冶屋: 命の恩人だ。礼に力のハンマーを渡そう。[>]
鍛冶屋: 東の海岸に岩で塞がれた洞窟がある。これで砕いて進め。
```
**イベント**: 会話終了時に `tryGivePowerHammer()` が呼ばれ、力のハンマーが授与される

### フェーズ 1: BLACKSMITH_RESCUED（力のハンマー授与後）
```
鍛冶屋: そのハンマーで東の岩を砕けるはずだ。[>]
鍛冶屋: 聖鉱石はきっとその奥に眠っている。頼んだぞ。
```
**条件**: `hasHammer = true` の場合に表示される特別なセリフ
